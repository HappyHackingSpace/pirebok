...
def test_finds_better_payload() -> None:
    ...
    def form_side_effect(payload, flag):  # type: ignore[no-untyped-def]
        ...
        call_count += 1
        if call_count == 1:
            return ("path-traversal", 0.9)
        return ("path-traversal", 0.3)

    mocks = _make_mock_metamaska(form_side_effect)
    with patch.dict(sys.modules, mocks):
        fuzzer = (
            FuzzerBuilder()
            .choice("GuidedRandomPathTraversalFuzzer")
            .threshold(0.5)
            .max_rounds(1)
            .round_size(5)
            .build()
        )
        fuzzer._mutation_round = lambda payload, size: {"..%2f..%2fetc/passwd"}  # type: ignore[assignment]
        result = fuzzer.fuzz("../../etc/passwd")
