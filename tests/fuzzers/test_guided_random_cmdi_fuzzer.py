...
def test_finds_better_payload() -> None:
    ...
    with patch.dict(sys.modules, mocks):
        fuzzer = (
            FuzzerBuilder()
            .choice("GuidedRandomCmdiFuzzer")
            .threshold(0.5)
            .max_rounds(1)
            .round_size(5)
            .build()
        )
        fuzzer._mutation_round = lambda payload, size: {"|${IFS}cat${IFS}/etc/passwd"}  # type: ignore[assignment]
        result = fuzzer.fuzz("; cat /etc/passwd")
